...
const AddRequest = () => {
    ...
    const handleAddRequest = async(e) =>
    {
        ...
        const requestDetails = {title, description, category, location, urgency, status};
        console.log(requestDetails);
        try 
        {
            const res = await axios.post("http://localhost:5000/addRequest", requestDetails , 
                {
                    headers:
                    {
        ...
        <form onSubmit={handleAddRequest} className="space-y-6">
          <div className="space-y-2">
            ...
            <input
              ...
              placeholder="Type Request Title" name="title" required
            />
          </div>
          <div className="space-y-2">
            <label className="block text-sm font-poppins font-semibold text-gray-700">
            ...
            <input
              type="text"
              className="w-full px-4 py-3 rounded-lg border border-gray-200 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all"
              placeholder="Type Request Category" name="category" required
            />
          </div>
          <div className="space-y-2">
            <label className="block text-sm font-poppins font-semibold text-gray-700">
            ...
            <input
              type="text"
              className="w-full px-4 py-3 rounded-lg border border-gray-200 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all"
              placeholder="Type Location" name="location" required
            />
          </div>
          <div className="space-y-2">
            <label className="block text-sm font-poppins font-semibold text-gray-700">
            ...
            <select name="urgency" required className="w-full px-4 py-3 rounded-lg border border-gray-200 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all bg-white appearance-none">
              <option>Low</option>
